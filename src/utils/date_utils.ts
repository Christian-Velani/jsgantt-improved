...
export const getMinDate = function (pList, pFormat, pMinDate) {
  ...
  vDate.setTime(
    (pMinDate && pMinDate.getTime()) || pList[0].getStart().getTime()
  );
...
export const getMaxDate = function (pList, pFormat, pMaxDate) {
  ...
  vDate.setTime(
    (pMaxDate && pMaxDate.getTime()) || pList[0].getEnd().getTime()
  );
...
export const getIsoWeek = function (pDate) {
  ...
  let daysFromFirstMonday = Math.round(
    (thisTime - firstMondayYearTime) / dayMiliseconds
  );
  let lastWeek = 99;
  let thisWeek = 99;
