...
export const getOffset = function (
  ...
  let vTaskRight =
    calculateCurrentDateOffset(curTaskStart, curTaskEnd) / oneHour;
  ...
  if (pFormat == "day") {
    ...
    vTaskRightPx = Math.ceil(
      (vTaskRight / (24 * 7)) * (pColWidth + WEEK_CELL_MARGIN_WIDTH) - 1
    );
...
export const fadeToolTip = function (pDirection, pTool, pMaxAlpha) {
  ...
  if (
    (vCurAlpha != pMaxAlpha && pDirection == 1) ||
...
export const updateFlyingObj = function (e, pGanttChartObj, pTimer) {
  ...
  /*
	...
	*/
  let outViewport =
    Math.abs(vOldX - vNewX) > screenX || Math.abs(vOldY - vNewY) > screenY;
