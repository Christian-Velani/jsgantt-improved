...
export const TaskItem = function (
  ...
  if (pStart != null && pStart != "") {
    vStart =
      pStart instanceof Date
        ? pStart
        : parseDateStr(
            document.createTextNode(pStart).data,
            vGantt.getDateInputFormat()
          );
  ...
  if (pEnd != null && pEnd != "") {
    vEnd =
      pEnd instanceof Date
        ? pEnd
        : parseDateStr(
            document.createTextNode(pEnd).data,
            vGantt.getDateInputFormat()
          );
    vGroupMinEnd = vEnd;
  }
  ...
  if (pPlanStart != null && pPlanStart != "") {
    vPlanStart =
      pPlanStart instanceof Date
        ? pPlanStart
        : parseDateStr(
            document.createTextNode(pPlanStart).data,
            vGantt.getDateInputFormat()
          );
  ...
  if (pPlanEnd != null && pPlanEnd != "") {
    vPlanEnd =
      pPlanEnd instanceof Date
        ? pPlanEnd
        : parseDateStr(
            document.createTextNode(pPlanEnd).data,
            vGantt.getDateInputFormat()
          );
  ...
  this.setChildRow = function (pRow) {
    if (
      typeof HTMLTableRowElement !== "function" ||
    ...
    if (template) {
      ...
      if (this.vShowTaskInfoEndDate == 1) {
        ...
        newNode(
          ...
          null,
          "gTaskLabel",
          this.vLangs[this.vLang]["enddate"] + ": "
        );
        newNode(
          vTmpDiv,
          "span",
      ...
      if (this.vShowTaskInfoComp == 1) {
        ...
        newNode(
          ...
          null,
          "gTaskLabel",
          this.vLangs[this.vLang]["completion"] + ": "
        );
      ...
      if (this.vShowTaskInfoLink == 1 && pTask.getLink() != "") {
        ...
        vTmpNode = newNode(
          ...
          null,
          "gTaskText",
          this.vLangs[this.vLang]["moreinfo"]
        );
  ...
  if (typeof templateStrOrFn === "function") {
    callback = () => {
      ...
      if (!strOrPromise || typeof strOrPromise === "string") {
        ...
        setupTemplate(
          this.vLangs[this.vLang]["tooltipLoading"] ||
            this.vLangs["en"]["tooltipLoading"]
        );
...
export const processRows = function (
  pList,
  pID,
  pRow,
  pLevel,
  ...
  if (pRow >= 0) {
    ...
    if (
      pList[pRow].getGroupMinEnd() != null &&
